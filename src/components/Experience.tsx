    ...
    <section id="experience" className="py-24 bg-background-secondary">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl font-orbitron font-bold mb-12 flex items-center">
          ...
          <span className="bg-gradient-warm bg-clip-text text-transparent ml-2">Journey</span>
        </h2>
          ...
          {experiences.map((exp, index) => (
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                {/* Company Logo */}
                <div className={`w-20 h-20 rounded-2xl ${exp.gradient} p-4 flex items-center justify-center group-hover:animate-pulse-glow`}>
                  <img
                    src={exp.logo}
                    ...
                    onError={(e) => {
                      ...
                      if (parent) {
                        parent.innerHTML = `<span class="text-white font-orbitron font-bold text-lg">${exp.company.split(' ').map(word => word[0]).join('')}</span>`;
                      }
                    }}
                  />
                </div>
